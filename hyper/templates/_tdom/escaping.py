...
def _format_safe(value: object, format_spec: str) -> str:
    ...
    assert format_spec == "safe"  # nosec B101 - format spec validation
    return Markup(value)  # nosec B704 - intentional API for marking trusted content as safe


def _format_unsafe(value: object, format_spec: str) -> str:
    """Convert a value to a plain string, forcing it to be treated as unsafe."""
    assert format_spec == "unsafe"  # nosec B101 - format spec validation
    return str(value)
